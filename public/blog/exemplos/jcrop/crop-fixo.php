<html>
	...
	<body>
		...
		<?php
			// limpa arquivos temporários ( <= 5 minutos )
			$extensoes 	= array( 'gif', 'png', 'jpg', 'jpeg' );
			$arquivos  	= scandir( '.' );
			$data_atual	= strtotime( date( 'YmdHis' ) . ' - 5 minutes ' );
			foreach( $arquivos as $arq )
			{
				$pathinfo 	= pathinfo( $arq );
				$extensao	= strtolower( isset( $pathinfo['extension'] ) ? $pathinfo['extension'] : NULL );
				if( $extensao && in_array( $extensao, $extensoes ) )
				{
					$data_arquivo	= filemtime( $arq );
					if( $data_arquivo <= $data_atual ) unlink( $arq );
				}
			}
				...
				$(function(){
					...
					$('#btn-crop').click(function(){
						$.post( 'crop.php', {
							...
							$('#debug').hide();
							$('#tit-jcrop').html('Feito!<br /><a href="crop-fixo.php">enviar outra imagem</a>');
						});
						return false;
					});
				});
